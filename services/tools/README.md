...
Gmail forwarding/deletion/modification, Contacts, Drive, other external SaaS integrations, durable provider idempotency, and multi-tool planning remain future work.
...
Calendar tools are `calendar.events.list`, `get`, `create`, `update`, and `delete`. Reads are `READ`/`NONE`/idempotent with `calendar.events.read`. Create and update are `WRITE`; delete is `DESTRUCTIVE`; all mutations are `REQUIRED`/`NON_IDEMPOTENT` with `calendar.events.write`. Create and time updates use offset-aware start/end, explicit IANA timezone, and a 24-hour duration maximum. Update requires an event ID and at least one allowlisted change, sends only requested PATCH fields, and cannot erase omitted provider fields. Delete accepts only an event ID and returns `{ eventId, deleted: true }`. Server-derived previews and exact-action proofs precede fixed primary-calendar POST/PATCH/DELETE calls. Mutations never retry. Attendees, recurrence, reminders, conferencing, descriptions, arbitrary metadata, and additional calendars are not implemented.

Gmail registers `gmail.messages.list`, `gmail.messages.get`, `gmail.messages.send`, and `gmail.messages.reply`. Reads require exact `gmail.messages.read`; send/reply require exact `gmail.messages.send`, risk `WRITE`, approval `REQUIRED`, and are non-idempotent. Send accepts one validated recipient, bounded subject, and bounded plain-text body. Reply accepts only message ID/body, then derives recipient and threading metadata from Gmail. The fixed `users/me` adapter constructs Base64URL MIME internally and performs at most one POST after approval. It never retries ambiguous sends and exposes no raw MIME, labels, attachments, CC/BCC, arbitrary headers, or provider response.

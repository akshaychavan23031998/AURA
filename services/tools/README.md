...
Calendar tools are `calendar.events.list`, `calendar.events.get`, and `calendar.events.create`. Reads remain `READ`, approval `NONE`, idempotent, and require exact `calendar.events.read`. Create is `WRITE`, approval `REQUIRED`, `NON_IDEMPOTENT`, and requires exact `calendar.events.write`. It accepts only summary (200 characters), offset-aware start/end, an explicit valid IANA timezone, and optional location (500 characters); duration is positive and at most 24 hours. Tool Service derives the approval preview and the allowlisted Google payload. The fixed-host client performs one primary-calendar POST after trusted approval proof and never retries. Outputs are normalized. Update/delete, attendees, recurrence, reminders, conferencing, descriptions, and additional calendars are not implemented.

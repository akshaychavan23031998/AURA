...
Gmail mutation tools, Contacts, Drive, other external SaaS integrations, durable idempotency, and multi-tool planning remain future work.
...
Phase 24 registers only `gmail.messages.list` and `gmail.messages.get`. Both are `READ`, approval `NONE`, idempotent, and require exact `gmail.messages.read`. List accepts at most 20 results and optional plain search terms up to 200 characters; Gmail operator syntax, pagination, account selection, and unknown fields are rejected. Get requires a bounded message ID. The fixed `users/me` adapter requests metadata format and normalizes identifiers, From/To/Cc/Subject/Date, and a bounded snippet. It exposes no raw MIME, bodies, labels, history IDs, attachments, or mutation capability.

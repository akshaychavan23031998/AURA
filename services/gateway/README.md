...
Google Calendar access is opt-in through `GOOGLE_CALENDAR_ENABLED`. Authorization Code + PKCE requests the official `calendar.readonly` scope for reads and `calendar.events` for event creation, plus offline access. AURA independently requires exact read/write permissions. Existing read-only credentials cannot create events and return `PROVIDER_REAUTH_REQUIRED` until Google re-consent. A 32-byte base64 `GOOGLE_PROVIDER_TOKEN_ENCRYPTION_KEY` protects refresh credentials bound to the stable Google subject. Gateway refreshes bounded access tokens and forwards them only over the internal Tool channel. `calendar.events.create` suspends through the existing exact-action approval workflow; one consumed approval permits at most one dispatch and no ambiguous write is retried.

...
Phase 45 adds the private `workflow:worker` process. `/run` records explicit execution authorization; proposal creation alone is not worker eligibility. Workers acquire a workflow-level PostgreSQL lease, heartbeat it, and fence executor advancement using an opaque owner and monotonically increasing generation. One workflow is active per process and intra-workflow execution remains sequential. Lease expiry invokes Phase 44 classification instead of permitting mutation replay. The HTTP process keeps worker mode disabled, active actor policy is reconstructed without JWT persistence, and provider access tokens remain ephemeral.
...
Phase 38 further rejects duplicate DOCX critical entries, macro-enabled package content types, external root document relationships, and unsafe XML declarations. The shared embedding client consumes response streams under a 256 KiB ceiling, performs no retry or redirect, and accepts only a configured origin-style HTTP(S) base URL without credentials, paths, query parameters, or fragments. None of these dependency failures weaken durable Memory/Knowledge persistence semantics or expose upstream details.

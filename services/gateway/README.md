...
Google Calendar read access is opt-in through `GOOGLE_CALENDAR_ENABLED`. It adds only Google's `calendar.readonly` scope to the existing Authorization Code + PKCE consent and requires a 32-byte base64 `GOOGLE_PROVIDER_TOKEN_ENCRYPTION_KEY`. Refresh credentials are bound to the authenticated user's stable Google subject, encrypted with AES-256-GCM, and never returned by an API. Gateway refreshes short-lived provider access tokens with a bounded request and forwards them only for `calendar.*` calls on the internal Tool channel. Revoked or missing credentials return `PROVIDER_REAUTH_REQUIRED`.

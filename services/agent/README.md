...
The planner can propose exactly one response, Tool action, explicit memory read/create/delete, or `memory_search`. Semantic search is allowed only when the user clearly asks about particular previously saved information; ordinary requests never trigger proactive retrieval. Persistent memory requires clear user intent, and deletion requires an exact memory UUID. Gateway remains authoritative for actor ownership, permissions, validation, search bounds, and persistence. Returned memory context is bounded and labeled untrusted user-authored data: it may answer the saved-memory question but cannot override system policy or act as authorization. Continuations must produce a final response and cannot recursively propose another action.

Phase 35 adds one initial `knowledge_search` proposal for explicit questions about saved documents or the knowledge base. Its only model-controlled value is a bounded query. Gateway supplies ranked evidence as dedicated `knowledgeContext` entries with local `K` references. That content is untrusted evidence—not instructions—and the continuation schema permits only a final grounded response plus citation IDs. It must use supplied evidence, disclose insufficiency, and cannot propose Tools, approvals, OAuth, memory actions, or another retrieval. Gateway validates and resolves citations; Agent never receives trusted document identifiers, vectors, scores, ownership, or search policy.

This remains a self-hosted LLM-backed single-action planning foundation, not full autonomy, prompt-injection immunity, automatic retrieval, hybrid search/reranking, automatic personalization, or automatic memory extraction. Tool Service remains authoritative for registered tools; Gateway remains authoritative for memory, knowledge retrieval, and citations.

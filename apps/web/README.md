...
When Calendar access is enabled, Google consent includes the official Calendar read and event-write scopes plus offline token issuance. The browser still receives only AURA session credentials; Google tokens never enter frontend JavaScript, Redux, storage, URLs, or voice events. Calendar creation uses the existing accessible `ApprovalCard`: merely rendering or reading an approval cannot execute, and voice phrases have no approval authority. Only an explicit authenticated Approve click can resume the exact persisted action.

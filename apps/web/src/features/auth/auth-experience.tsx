...
type AuthenticatedSection =
  "conversation" | "memory" | "knowledge" | "workflows" | "accounts";
...
export function AuthExperience() {
  ...
  const workflowApi = useMemo(
    () => new WorkflowApi(authenticatedFetch),
    [authenticatedFetch],
  );
  ...
  if (auth.status === "authenticated") {
    ...
    if (token === undefined) {
      return <AuthPanel status="session-expired" />;
    }
    ...
    return (
      <div className="authenticated-shell">
        <nav className="product-nav" aria-label="AURA sections">
          ...
          {(["conversation", "memory", "knowledge", "workflows"] as const).map(
            (item) => (
              <button
                key={item}
                type="button"
        ...
        {section === "workflows" && <WorkflowPanel api={workflowApi} />}

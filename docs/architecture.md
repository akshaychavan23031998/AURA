...
`calendar.events.create` is the sole Calendar write action. Tool Service validates its allowlisted summary/start/end/timezone/location contract, marks it `WRITE`, `REQUIRED`, and `NON_IDEMPOTENT`, and generates its preview from validated input. Gateway persists the exact action digest; only explicit authenticated browser approval consumes it and permits one Tool dispatch. Preparation, approval reads, voice phrases, rejection, expiry, and card rendering cannot cross the fixed Google Calendar POST boundary. A lost response after dispatch is ambiguous and is not retried. Update/delete, attendees, recurrence, conferencing, and multiple calendars are not implemented.

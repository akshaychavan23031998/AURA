...
The production registry contains three local tools, five Calendar tools, and two read-only Gmail tools. Tool Service remains authoritative for schemas and exact permissions; Google credentials remain Gateway-owned. Gmail writes, Contacts, Drive, additional providers, persisted idempotency, and Kafka execution events remain planned.
...
Gmail is independently gated by `GOOGLE_GMAIL_ENABLED`. It adds only `https://www.googleapis.com/auth/gmail.readonly`; Tool Service requires exact `gmail.messages.read`. Gateway verifies that scope before forwarding an ephemeral access token internally. The Gmail adapter is fixed to `https://gmail.googleapis.com/gmail/v1/users/me/messages`, supports bounded list/get metadata calls only, and normalizes From/To/Cc/Subject/Date/snippet fields. Provider tokens, raw MIME, arbitrary headers, attachments, and raw responses never enter Agent, Web, Voice, ToolResult, or logs.

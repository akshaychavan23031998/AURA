...
The production registry contains three local tools, five Calendar tools, and four Gmail tools. Tool Service remains authoritative for schemas and exact permissions; Google credentials remain Gateway-owned. Gmail send/reply reuse the persistent approval boundary and never retry. Contacts, Drive, additional providers, persisted idempotency, and Kafka execution events remain planned.

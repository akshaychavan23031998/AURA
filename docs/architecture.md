...
Phase 43 permits only structured scalar Tool-input references (`fromStep` plus an allowlisted export `field`). Gateway validation requires an explicit transitive dependency, compatible primitive source/destination types, and no more than eight references per step. Resolution occurs after atomic claim from the immutable attempt-one result of a same-workflow SUCCEEDED ancestor. Stored JSONB proposals are never rewritten. Approval preparation and its digest receive the resolved exact input, and approval resume never resolves again. Generic templates, nested paths, expressions, memory/knowledge exports, branches, loops, retries, and recovery remain absent.

Phase 44 adds an explicit owner-scoped recovery operation after a server-controlled 60-second stale interval. Attempt-one execution checkpoints are persisted before external dispatch and again when a result returns. Safe Memory/Knowledge reads and ToolDefinition-declared idempotent Tools can re-drive the same row; a persisted result is finalized without execution. Non-idempotent work at or beyond the dispatch boundary transitions to first-class `RECOVERY_REQUIRED`/`AMBIGUOUS`, accurately stating that the provider may have received it and forbidding downstream unlock or replay. Pending approvals are reused, consumed approvals execute their stored exact action, and resolved references are never recomputed after approval. Startup performs no scan or recovery; there are no retries, workers, leases, heartbeats, schedules, or external exactly-once claim.

Phase 45 adds a separate `workflow:worker` runtime. It polls only when explicitly enabled, acquires one workflow-level PostgreSQL lease, heartbeats it, and supplies its opaque owner plus monotonically increasing generation as a fencing guard to the existing executor. Only workflows explicitly authorized through `/run` are eligible. Multiple workers can process separate workflows, while execution inside each workflow remains sequential. Expired leases are handled through the Phase 44 recovery classifier; `RECOVERY_REQUIRED`, approval-waiting, cancelled, and terminal workflows are not dispatched. Active actor identity and server policy are reconstructed from PostgreSQL without storing browser JWTs, while native permissions and encrypted actor-owned provider credentials remain authoritative.

...
V1 registers the three local utilities, five Google Calendar tools, and the read-only `gmail.messages.list`/`gmail.messages.get` capabilities through the sealed Tool Registry and centralized policy pipeline. Gmail writes, Contacts, Drive, and other SaaS integrations remain unimplemented.
...
The Google Calendar V1 set is `calendar.events.list`, `get`, `create`, `update`, and `delete`, all restricted to the authenticated user's primary calendar. Phase 24 adds only `gmail.messages.list` and `gmail.messages.get`, using `gmail.readonly`, exact `gmail.messages.read`, fixed `users/me` endpoints, and normalized headers/snippets. Gmail scope is opt-in through `GOOGLE_GMAIL_ENABLED`; older credentials require re-consent. Gateway protects encrypted refresh credentials and issues bounded access tokens only to Tool Service. Gmail send/reply/forward/delete/modify, drafts, attachments, Contacts, and Drive remain unimplemented.

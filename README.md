...
Knowledge ingestion is an account-data API, not an Agent Tool. Agent, Tool Service, and Voice receive no document, chunk, or vector access. List responses contain metadata only, raw content is returned only by an explicit owner-scoped GET, and request logs contain neither titles nor document/chunk content. Phase 33 adds no semantic document search, Agent RAG, citations, file/URL ingestion, or automatic indexing worker.

...
Phase 44 adds explicit crash recovery through `POST /api/v1/workflows/:workflowId/recover`. Gateway-owned durable checkpoints distinguish claimed/prepared work, the external dispatch boundary, returned results, approval waits, finalization, and ambiguous mutations. Stale safe reads and ToolDefinition-declared idempotent Tools may resume using attempt one; known persisted results reconcile without dispatch. A possibly dispatched non-idempotent action becomes `RECOVERY_REQUIRED` and is never blindly replayed. Recovery is not automatic at startup and adds no generic retry, worker, lease, schedule, UI, or Voice authority.

Phase 45 adds an opt-in dedicated Gateway worker. Explicitly run-authorized workflows are acquired through PostgreSQL workflow leases with opaque owners, monotonically increasing fencing generations, and heartbeats. One workflow runs per worker process and multiple processes may handle separate workflows. Lease expiry never proves an external mutation did not occur: Phase 44 classification remains authoritative, attempt one remains the only attempt, and ambiguous work is never replayed. The HTTP Gateway does not poll, JWTs are not persisted, provider access tokens remain ephemeral, and there is still no scheduler, retry engine, Redis, Kafka, or Temporal.

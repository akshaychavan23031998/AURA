...
The Google Calendar V1 set is `calendar.events.list`, `get`, `create`, `update`, and `delete`, all restricted to the authenticated user's primary calendar. List/get are `READ` with no approval; create/update are `WRITE`, delete is `DESTRUCTIVE`, and every mutation requires exact `calendar.events.write` plus explicit browser approval. Google uses only `calendar.readonly` and `calendar.events`; older read-only credentials require re-consent. Gateway protects provider credentials and exact persisted actions, while Tool Service builds fixed POST/PATCH/DELETE requests. One approval permits at most one mutation dispatch, and ambiguous outcomes are never retried. Attendees, recurrence, conferencing, Gmail, Drive, Contacts, background sync, and provider tokens in browser or Agent state remain unimplemented.

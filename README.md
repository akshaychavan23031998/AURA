...
Phase 21 adds `calendar.events.list` and `calendar.events.get` as strictly read-only Google Calendar tools. When explicitly enabled, Google consent adds only `calendar.readonly` and requests an offline refresh credential. Gateway encrypts refresh credentials at rest, refreshes bounded short-lived access tokens server-side, and supplies them only over the authenticated internal Tool channel. Calendar writes, Gmail, Drive, Contacts, background sync, and provider tokens in browser or Agent state are not implemented.

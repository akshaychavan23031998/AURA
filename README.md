...
Phase 21 adds `calendar.events.list` and `calendar.events.get` as strictly read-only Google Calendar tools. Phase 22 adds only `calendar.events.create`: a `WRITE`, `NON_IDEMPOTENT` action requiring `calendar.events.write` and explicit browser approval. Google consent uses the separate official `calendar.readonly` and `calendar.events` scopes; an older read-only credential must be re-consented before creation. Gateway encrypts refresh credentials at rest and supplies bounded access tokens only over the internal Tool channel. The exact validated event is approval-bound and dispatched at most once; ambiguous provider outcomes are never retried. Update/delete, attendees, recurrence, conferencing, Gmail, Drive, Contacts, background sync, and provider tokens in browser or Agent state remain unimplemented.
